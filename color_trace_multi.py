...
"""
    ...
    parser.add_argument('-t',
                        '--tracemasksdir', default=None,
                        help='directory containing the quantized layer masks generated by Separo')
...
def q1_job(q2, total, layers, settings, findex, input, output, tracemasksdir):
...
"""
    ...
    try:
        ...
        for i, color in enumerate(palette):
            q2.put({ 'width': width, 'color': color, 'palette': palette, 'reduced': this_reduced, 'output': output, 'findex': findex, 'cindex': i, 'tracemasksdir': tracemasksdir })
...
def mask2ppm(mask, layer_ppm):
    ...
    command = '"{convert}" "{mask}" -resize 200% -morphology Open Disk:1 -threshold 80% "{layer}"'.format(
...
"""
    ...
    try:
        ...
        if tracemasksdir is not None:
            separo_isolated_path = os.path.join(tracemasksdir, '%s.png' % color.lower()[1:])
            print('Replacing %s with %s' % (separo_isolated_path, this_layer))
...
def color_trace_multi(inputs, outputs, colors, processcount, tracemasksdir, quantization='mc', dither=None,
...
"""
    ...
    try:
        ...
        for index, (i, o) in enumerate(zip(inputs, outputs)):
            ...
            q1.put({ 'input': i, 'output': o, 'findex': index, 'tracemasksdir': tracemasksdir })
